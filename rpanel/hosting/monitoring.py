...
def get_resource_usage(website_name, period="24h"):
    ...
    logs = frappe.get_all(
        "Resource Usage Log",
        filters={"website": website_name, "timestamp": [">=", start_time]},
        fields=[
            "timestamp",
            "cpu_usage",
            "memory_usage",
    ...
    if logs:
        ...
        total_bandwidth = sum(
            (log.bandwidth_in or 0) + (log.bandwidth_out or 0) for log in logs
        )
...
def get_uptime_stats(website_name, period="7d"):
    ...
    checks = frappe.get_all(
        "Uptime Check",
        filters={"website": website_name, "check_time": [">=", start_time]},
    ...
    if checks:
        ...
        avg_response_time = (
            sum(check.response_time or 0 for check in checks if check.is_up) / up_count
            if up_count > 0
            else 0
        )
    else:
    ...
    try:
        if os.path.exists(error_log):
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "logs": lines, "count": len(lines)}
