        ...
        try:
            ...
            try:
                with open(sql_path, "r") as sql_file:
                    subprocess.run(
                        ["sudo", "mysql", db_name], stdin=sql_file, check=True
                    )
...
?>"""
            ...
            subprocess.run(
                ["sudo", "chmod", "-R", "775", os.path.join(target_dir, "logs")],
                check=True,
            )
            subprocess.run(
                ["sudo", "chmod", "-R", "775", os.path.join(target_dir, "temp")],
                check=True,
            )
    ...
    def run_command(self, command, description):
        try:
            ...
            frappe.throw(
                f"{description} Failed.<br>Error Code: {e.returncode}<br><pre>{
                    e.stderr
                }</pre>"
            )
