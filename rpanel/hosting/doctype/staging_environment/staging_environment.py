...
from rpanel.hosting.mysql_utils import (
    run_mysql_command,
    run_mysqldump,
    run_mysql_restore,
)
...
def create_staging(production_website_name: str) -> dict:
    ...
    sys.stderr.write(
        f"[TRACE] create_staging trace_id={getattr(getattr(__import__('frappe'), 'local', object()), 'trace_id', 'n/a')}\n"
    )
    ...
    try:
        ...
        create_staging_database(
            production.db_name, staging_db, production.db_user, production.db_password
        )
...
@frappe.whitelist()
def sync_to_staging(
    staging_name: str, sync_database: str = True, sync_files: str = True
) -> dict:
    """Sync production to staging"""
    sys.stderr.write(
        f"[TRACE] sync_to_staging trace_id={getattr(getattr(__import__('frappe'), 'local', object()), 'trace_id', 'n/a')}\n"
    )
...
def push_to_production(staging_name: str) -> dict:
    ...
    sys.stderr.write(
        f"[TRACE] push_to_production trace_id={getattr(getattr(__import__('frappe'), 'local', object()), 'trace_id', 'n/a')}\n"
    )
    staging = frappe.get_doc("Staging Environment", staging_name)
    production = frappe.get_doc("Hosted Website", staging.production_website)
...
def delete_staging(staging_name: str) -> dict:
    ...
    sys.stderr.write(
        f"[TRACE] delete_staging trace_id={getattr(getattr(__import__('frappe'), 'local', object()), 'trace_id', 'n/a')}\n"
    )
    staging = frappe.get_doc("Staging Environment", staging_name)
...
def create_staging_database(prod_db, staging_db, db_user, db_password):
    ...
    run_mysqldump(
        database=prod_db, output_file=dump_file, user=db_user, password=db_password
    )

    # Import to staging database - Secure: password hidden
    run_mysql_restore(
        database=staging_db, input_file=dump_file, user=db_user, password=db_password
    )
...
def sync_staging_database(source_db, target_db):
    ...
    run_mysqldump(database=source_db, output_file=dump_file, user="root", as_sudo=True)

    # Import to target
    run_mysql_restore(
        database=target_db, input_file=dump_file, user="root", as_sudo=True
    )
