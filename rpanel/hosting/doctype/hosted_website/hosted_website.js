    ...
    refresh: function (frm) {
        ...
        if (!frm.is_new()) {
            ...
            frm.add_custom_button(__('Pull Latest'), function () {
                frappe.call({
                    method: 'rokct.hosting.git_manager.pull_latest',
...
function open_file_manager(frm) {
    ...
    function load_files(path = '') {
        ...
        frappe.call({
            method: 'rokct.hosting.file_manager.get_file_list',
            args: {
    ...
    function render_file_browser(data, dialog) {
        ...
        window.file_manager_download = function (path) {
            window.open(`/api/method/rokct.hosting.file_manager.download_file?website_name=${frm.doc.name}&file_path=${encodeURIComponent(path)}`);
        };
        ...
        window.file_manager_delete = function (path, is_dir) {
            frappe.confirm(
                ...
                function () {
                    frappe.call({
                        method: 'rokct.hosting.file_manager.delete_file',
                        args: {
                            website_name: frm.doc.name,
                            file_path: path
                        },
                    ...
                    if (values.file) {
                        ...
                        frappe.call({
                            ...
                            callback: function (r) {
                                if (r.message) {
                                    frappe.call({
                                        method: 'rokct.hosting.file_manager.upload_file',
                                        args: {
                                            website_name: frm.doc.name,
                                            path: current_path,
        ...
        window.file_manager_new_folder = function () {
            frappe.prompt({
                ...
                frappe.call({
                    method: 'rokct.hosting.file_manager.create_directory',
                    args: {
                        website_name: frm.doc.name,
                        path: current_path,
        ...
        window.file_manager_edit = function (path) {
            frappe.call({
                method: 'rokct.hosting.file_manager.read_file',
                args: {
                    website_name: frm.doc.name,
                    file_path: path
                },
                callback: function (r) {
                    if (r.message) {
                        const edit_dialog = new frappe.ui.Dialog({
                            ...
                            primary_action: function (values) {
                                frappe.call({
                                    method: 'rokct.hosting.file_manager.save_file',
                                    args: {
                                        website_name: frm.doc.name,
                                        file_path: path,
...
function show_backup_dialog(frm) {
    let d = new frappe.ui.Dialog({
        ...
        primary_action: function (values) {
            frappe.call({
                method: 'rokct.hosting.doctype.site_backup.site_backup.create_backup',
                args: {
                    website_name: frm.doc.name,
...
function show_git_clone_dialog(frm) {
    let d = new frappe.ui.Dialog({
        ...
        primary_action: function (values) {
            frappe.call({
                method: 'rokct.hosting.git_manager.clone_repository',
                args: {
                    website_name: frm.doc.name,
...
function show_deployment_history(frm) {
    frappe.call({
        method: 'rokct.hosting.git_manager.get_deployment_history',
        args: { website_name: frm.doc.name },
        callback: function (r) {
            if (r.message && r.message.success) {
                ...
                window.rollback_to_commit = function (hash) {
                    frappe.confirm(
                        ...
                        function () {
                            frappe.call({
                                method: 'rokct.hosting.git_manager.rollback_deployment',
...
function show_webhook_dialog(frm) {
    frappe.call({
        method: 'rokct.hosting.git_manager.setup_webhook',
        args: { website_name: frm.doc.name },
        callback: function (r) {
            if (r.message && r.message.success) {
                let d = new frappe.ui.Dialog({
...
function load_log_content(dialog, frm, silent = false) {
    ...
    let method = 'rokct.hosting.log_viewer.get_nginx_access_log';
    if (log_type === 'nginx_error') method = 'rokct.hosting.log_viewer.get_nginx_error_log';
    else if (log_type === 'php_error') method = 'rokct.hosting.log_viewer.get_php_error_log';
    else if (log_type === 'application') method = 'rokct.hosting.log_viewer.get_application_log';
