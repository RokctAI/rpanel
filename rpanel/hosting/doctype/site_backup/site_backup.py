...
from rpanel.hosting.postgres_utils import run_pg_dump, run_pg_restore
...
class SiteBackup(Document):
        ...
        try:
            ...
            if security_settings.enable_backup_encryption:
                try:
                    ...
                    frappe.log_error(
                        f"Backup encryption failed: {str(e)}", "Backup Encryption Error"
                    )
    ...
    def create_full_backup(self, website, backup_dir, backup_name):
        ...
        cmd = f"tar -czf {backup_file} -C {website.site_path} . -C {backup_dir} {
    ...
    def dump_database(self, website, backup_dir, backup_name):
        """Dump database to SQL file (supports MariaDB and PostgreSQL)"""
        db_file = os.path.join(backup_dir, f"{backup_name}_db.sql")
        # Secure: Password hidden from process list
        if getattr(website, "db_engine", "MariaDB") == "PostgreSQL":
            run_pg_dump(
    ...
    def upload_to_gcs(self, backup_file):
        ...
        return (
            f"gs://{settings.get('gcs_bucket')}/backups/{os.path.basename(backup_file)}"
        )
