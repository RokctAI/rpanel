...
def get_columns():
    return [
        ...
        {"fieldname": "is_up", "label": _("Status"), "fieldtype": "Data", "width": 100},
        {
...
def get_chart_data(data, filters):
    ...
    uptime_values = [
        (hourly_data[h]["up"] / (hourly_data[h]["up"] + hourly_data[h]["down"]) * 100)
        if (hourly_data[h]["up"] + hourly_data[h]["down"]) > 0
        else 0
        for h in labels
    ]
    ...
    chart = {
        "data": {
            ...
            "datasets": [{"name": "Uptime (%)", "values": uptime_values[-24:]}],
        },
        "type": "line",
        "colors": ["#4CAF50"],
