...
def get_chart_data(data, filters):
    ...
    cpu_values = [
        sum(hourly_data[h]["cpu"]) / len(hourly_data[h]["cpu"])
        if hourly_data[h]["cpu"]
        else 0
        for h in labels
    ]
    memory_values = [
        sum(hourly_data[h]["memory"]) / len(hourly_data[h]["memory"])
        if hourly_data[h]["memory"]
        else 0
        for h in labels
    ]
    ...
    chart = {
        "data": {
            ...
            "datasets": [
                {"name": "CPU Usage (%)", "values": cpu_values[-24:]},
                {"name": "Memory Usage (MB)", "values": memory_values[-24:]},
            ],
        },
        "type": "line",
