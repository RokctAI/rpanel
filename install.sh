...
run_quiet "Installing RPanel app" sudo -u frappe -i bash -c "set -e; export PATH=\"\$PATH:/home/frappe/.local/bin\"; cd /home/frappe/frappe-bench; if [ ! -d \"apps/rpanel\" ]; then bench get-app https://github.com/RokctAI/rpanel.git $TAG_OPTION --skip-assets; else cd apps/rpanel && git fetch --tags && ([ -n \"$LATEST_TAG\" ] && git checkout \"$LATEST_TAG\" || git pull); cd ../..; fi; if [ ! -d \"sites/${DOMAIN_NAME:-rpanel.local}\" ]; then bench new-site \"${DOMAIN_NAME:-rpanel.local}\" --admin-password admin --db-root-password \"$DB_ROOT_PASS\" $([[ \"$DB_TYPE\" == \"postgres\" ]] && echo \"--db-type postgres\") --force; fi; bench --site \"${DOMAIN_NAME:-rpanel.local}\" install-app rpanel; bench setup redis"
